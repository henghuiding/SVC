function Y = vl_nnloss_new(X,c,dzdy,varargin)
...
switch lower(opts.loss)
  case {'classerror', 'log', 'softmaxlog', 'idfsoftmaxlog', 'mhinge', 'mshinge'}
    binary = false ;
...
switch lower(opts.loss)
  case {'log', 'softmaxlog', 'idfsoftmaxlog' 'mhinge', 'mshinge'}
...
if nargin <= 2 || isempty(dzdy)
  switch lower(opts.loss)
    ...
    case 'idfsoftmaxlog'
        ...
        t = Xmax + log(sum(ex,3)) - X(ci) ;
        w = classWeights(c+1);
        instanceWeights = instanceWeights .* w;
    case 'mhinge'
      t = max(0, 1 - X(ci)) ;
    case 'mshinge'
      Q = X ;
