function stats = fcn_train_dag(net, imdb, getBatch, varargin)
...
% -------------------------------------------------------------------------
f = net.getParamIndex('Paired_Peer_f_1') ;
mask=load('mask.mat'); mask=mask.mask;
peer_conv_der=gather(permute(net.params(f).der, [1 2 4 3]));
peer_conv_der=bsxfun(@times,mask,peer_conv_der);
net.params(f).der=gpuArray(permute(peer_conv_der,[1 2 4 3]));
...
% mask=abs(mask);
% peer_conv=gather(permute(net.params(f).value, [1 2 4 3]));
% peer_conv=bsxfun(@times,mask,peer_conv);
% net.params(f).value=gpuArray(permute(peer_conv,[1 2 4 3]));
