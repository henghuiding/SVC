function AutotestIFCN_PascalContext_multiscale(networkfolder)
% run(fullfile(fileparts(mfilename('fullpath')), '../matconvnet-1.0-beta16/matlab/vl_setupnn.m')) ;
% addpath(genpath('../toolbox'));
addpath(genpath('utils'));
baseDir = '/home/hhding/sharedLocal/datasets/VOC2010/';

%% General Configuration
nClass = 59;
imageSize = [256 288 320 352 384 448 480 512 544 576 608 640 672 704 736 800 832 864 896];
...
for i = 1:nImgs
    ...
    if i == 1 || mod(i,200) == 0
        print = true;
        fprintf('Labeling testing multi-scale images %s: %d/%d\n',imgIds{i}, i,nImgs);
    end
...
%             inputs = {input_name, I_, 'mask1', mask1};            
            tic;
